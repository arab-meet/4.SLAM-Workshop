...
These methods are suitable for small-scale environments but face challenges when scaling to larger spaces.
...
Graph-based SLAM techniques represent the SLAM problem as a graph, where nodes represent robot poses and landmarks, and edges represent constraints or measurements between them.

Graph-based SLAM has gained popularity for its efficiency in large-scale environments, especially in loop closure detection and optimization tasks.

You can check these approaches from these links:
...
These techniques is designed to improve SLAM performance in dynamic environments and under challenging conditions where traditional methods may struggle.
